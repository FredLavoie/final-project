    ...
    render() {
     ...
     const decimalPrice = parseFloat(Math.round(price * 100) / 100).toFixed(2);
      ...
      <tr>
          ...
          <td><span style={{display: "flex", alignContent: "center", justifyContent: "space-evenly"}}><button className="waves-effect btn-small red" onClick={() => this.props.deleteCartItem(this.props.shoppingcart.deal_id)}><i className="material-icons right">clear</i>remove</button></span></td>
      </tr>
      </tbody>     
      )
    }
